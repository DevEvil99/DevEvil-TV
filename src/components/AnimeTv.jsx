...
const AnimeTv = () => {
  ...
  useEffect(() => {
    const fetAnimeMovies = async () => {
      try {
        const animeData = await getAnimeTv(currentPage);
  ...
  return (
    <section className="movies" id='animetv'>

      <div className="filter-bar">
        <h1>Anime TV Shows <i className="fa-solid fa-paw"></i></h1>
      </div>
      {/* Movies grid */}
      <div className="movies-grid">
        {limit.map((anime) => (
 <Link to={`/tv/${anime.id}`}  key={anime.id}>
